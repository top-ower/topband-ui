<template>
  <span>
    <transition
      ...
      <div
        class="top-popover top-popper"
        :class="[popperClass, content && 'top-popover--plain']"
        ref="popper"
      ...
      >
        <div class="top-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, 'top-popover__reference');
