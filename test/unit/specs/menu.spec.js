...
describe('Menu', () => {
  ...
  it('create', done => {
    vm = createVue({
      template: `
        <top-menu>
          <top-menu-item index="1" ref="item1">处理中心</top-menu-item>
          <top-menu-item index="2" ref="item2">订单管理</top-menu-item>
        </top-menu>
      `
    }, true);
  ...
  it('menu-item click', done => {
    vm = createVue({
      template: `
        <top-menu>
          <top-menu-item @click="onMenuItemClick" index="1" ref="item1">处理中心</top-menu-item>
          <top-menu-item index="2" ref="item2">订单管理</top-menu-item>
        </top-menu>
      `,
      methods: {
  ...
  it('menu-item disabled', done => {
    vm = createVue({
      template: `
        <top-menu default-active="2">
          <top-menu-item index="1" ref="item1" disabled>处理中心</top-menu-item>
          <top-menu-item index="2" ref="item2">订单管理</top-menu-item>
        </top-menu>
      `
    }, true);
  ...
  describe('default active', () => {
    ...
    it('dynamic active', done => {
      vm = createVue({
        template: `
          <top-menu :default-active="active">
            <top-menu-item index="1" ref="item1">active watch处理中心</top-menu-item>
            <top-menu-item index="2" ref="item2">active watch订单管理</top-menu-item>
          </top-menu>
        `,
        data() {
          return {
    ...
    it('vertical submenu item active', done => {
      vm = createVue({
        template: `
          <div>
            <top-menu default-active="2-2">
              <top-menu-item index="1" ref="item1">处理中心</top-menu-item>
              <top-submenu index="2" ref="submenu">
                <template slot="title">我的工作台</template>
                <top-menu-item index="2-1">选项1</top-menu-item>
                <top-menu-item index="2-2" ref="submenuItem2">选项2</top-menu-item>
                <top-menu-item index="2-3">选项3</top-menu-item>
              </top-submenu>
              <top-menu-item index="3">订单管理</top-menu-item>
            </top-menu>
          </div>
        `
      }, true);
  ...
  describe('submenu', function() {
    it('toggle', done => {
      ...
      submenu.$el.querySelector('.top-submenu__title').click();
      setTimeout(_ => {
    ...
    it('default opened', done => {
      vm = createVue({
        template: `
          <top-menu :default-openeds="defaultOpeneds">
            <top-menu-item index="1">default opened处理中心</top-menu-item>
            <top-submenu index="2" ref="submenu1">
              <template slot="title">default opened我的工作台</template>
              <top-menu-item index="2-1">选项1</top-menu-item>
              <top-menu-item index="2-2" ref="submenu1Item2">选项2</top-menu-item>
              <top-menu-item index="2-3">选项3</top-menu-item>
            </top-submenu>
            <top-submenu index="3" ref="submenu2">
              <template slot="title">default opened订单管理</template>
              <top-menu-item index="3-1">选项1</top-menu-item>
              <top-menu-item index="3-2" ref="submenu2Item2">选项2</top-menu-item>
    ...
    it('disabled', done => {
      vm = createVue({
        template: `
          <top-menu>
            <top-menu-item index="1" ref="item1">处理中心</top-menu-item>
            <top-submenu index="2" ref="submenu" disabled>
              <template slot="title">我的工作台</template>
              <top-menu-item index="2-1">选项1</top-menu-item>
              <top-menu-item index="2-2" ref="submenuItem2">选项2</top-menu-item>
              <top-menu-item index="2-3">选项3</top-menu-item>
  ...
  it('unique-opened', done => {
    vm = createVue({
      template: `
        <top-menu unique-opened default-active="2-2">
    ...
    vm.$refs.submenu2.$el.querySelector('.top-submenu__title').click();
    setTimeout(_ => {
  ...
  it('horizontal mode', done => {
    ...
    expect(vm.$el.classList.contains('top-menu--horizontal')).to.be.true;
    var submenu = vm.$refs.submenu;
    triggerEvent(submenu.$el, 'mouseenter');
    setTimeout(_ => {
      expect(document.body.querySelector('.top-menu--popup').parentElement.style.display).to.not.ok;
      done();
    }, 500);
  });
  it('menu trigger click', done => {
    vm = createVue({
      template: `
        <top-menu mode="horizontal" menu-trigger="click">
          <top-menu-item index="1">处理中心</top-menu-item>
          <top-submenu index="2" ref="submenu">
            <template slot="title">我的工作台</template>
            <top-menu-item index="2-1">选项1</top-menu-item>
    ...
    expect(vm.$el.classList.contains('top-menu--horizontal')).to.be.true;
    var submenu = vm.$refs.submenu;
    var triggerElm = submenu.$el.querySelector('.top-submenu__title');
    ...
    setTimeout(_ => {
      ...
      triggerElm.click();
      setTimeout(_ => {
        expect(document.body.querySelector('.top-menu--popup').parentElement.style.display).to.be.equal('none');
        done();
      }, 1000);
    }, 500);
  });
  it('menu group', () => {
    vm = createVue({
      template: `
        <top-menu mode="vertical" default-active="1">
          <top-menu-item-group title="分组一" ref="group1">
            <top-menu-item index="1"><i class="top-icon-message"></i>导航一</top-menu-item>
            <top-menu-item index="2"><i class="top-icon-message"></i>导航二</top-menu-item>
          </top-menu-item-group>
          <top-submenu index="5">
            <template slot="title">导航五</template>
            <top-menu-item-group title="分组二">
              <top-menu-item index="5-1">选项1</top-menu-item>
              <top-menu-item index="5-2">选项2</top-menu-item>
            </top-menu-item-group>
          </top-submenu>
        </top-menu>
      `
    }, true);
    expect(vm.$refs.group1.$el.querySelector('.top-menu-item-group__title').innerText).to.be.equal('分组一');
  });
  it('dynamic menus, issue 9092', done => {
    ...
    setTimeout(_ => {
      ...
      setTimeout(_ => {
        expect(vm.$el.querySelector('.top-menu-item.is-active').innerText).to.equal('new');
        done();
      }, 20);
    }, 100);
  });
